#!/usr/bin/env python3
"""
Google Gemini API-based Assignment Evaluator
Uses AI to intelligently evaluate assignments with detailed feedback
"""
...
class GeminiEvaluator:
    def __init__(self, api_key: str = None):
        """Initialize the Gemini evaluator with API key"""
...
"""
    ...
    def evaluate_assignment(self, zip_path: str, assignment_type: str = "snowflake", 
        ...
        """Evaluate an assignment using Gemini AI"""
        
        try:
            # Extract files
            files_content = self.extract_zip_contents(zip_path)
            ...
            prompt = self.create_evaluation_prompt(assignment_type, files_content, assignment_brief)
            
            # Generate evaluation using Gemini
            response = self.model.generate_content(prompt)
            
            # Parse JSON response with multiple fallback strategies
            evaluation_result = self.parse_json_with_fallbacks(response.text)
            
            if evaluation_result:
                evaluation_result['files_analyzed'] = list(files_content.keys())
                evaluation_result['evaluation_timestamp'] = datetime.now().isoformat()
                evaluation_result['assignment_type'] = assignment_type
                return evaluation_result
            else:
                # Fallback: try to extract structured information from text
                return self.parse_text_response(response.text, files_content, assignment_type)
    ...
    def parse_text_response(self, text: str, files_content: Dict[str, str], 
        ...
        """Parse text response when JSON parsing fails - return list of notebooks"""
        
        # Analyze files to provide better feedback
        ...
        readme_files = [f for f in files_content.keys() if 'readme' in f.lower()]
        
        # Create notebook analysis as a list
        notebook_analysis = []
        for notebook in notebook_files:
            content = files_content[notebook]
            # Basic analysis of notebook content
            has_code = 'import' in content or 'def ' in content or 'class ' in content
            has_markdown = '#' in content or 'markdown' in content.lower()
            has_output = 'output' in content.lower() or 'result' in content.lower()
            
            # Create basic cell analysis
            cells = []
            if has_code:
                cells.append({
                    "cell_index": 0,
                    "cell_type": "code",
                    "cell_score": 7,
                    "feedback": "Code detected but needs detailed evaluation",
                    "requirements_covered": ["Basic implementation"]
                })
            if has_markdown:
                cells.append({
                    "cell_index": 1,
                    "cell_type": "markdown",
                    "cell_score": 6,
                    "feedback": "Documentation present but needs review",
                    "requirements_covered": []
                })
            
            notebook_analysis.append({
                "filename": notebook,
                "overall_score": 70 if has_code else 30,
                "overall_feedback": "Basic evaluation completed - detailed analysis needed",
                "cells": cells
            })
        
        # If no notebooks found, create a placeholder
        if not notebook_analysis:
            notebook_analysis.append({
                "filename": "unknown.ipynb",
                "overall_score": 0,
                "overall_feedback": "No notebooks detected in submission",
                "cells": []
            })
    ...
    def get_assignment_brief(self, assignment_type: str) -> Dict:
        """Get assignment brief for specified type"""
    ...
    def evaluate(self, solution_path: str, assignment_brief: dict) -> dict:
        """Public method for Streamlit app: evaluate solution zip and brief."""
        # Try to infer assignment type from brief, fallback to 'snowflake'
        assignment_type = assignment_brief.get('type', 'snowflake') if assignment_brief else 'snowflake'
        return self.evaluate_assignment(solution_path, assignment_type, assignment_brief)

    def get_prompt(self, solution_path: str, assignment_brief: dict) -> str:
        """Return the full evaluation prompt that would be sent to Gemini."""
        assignment_type = assignment_brief.get('type', 'snowflake') if assignment_brief else 'snowflake'
        files_content = self.extract_zip_contents(solution_path)
        return self.create_evaluation_prompt(assignment_type, files_content, assignment_brief)

# Test function
def test_gemini_evaluator():
    """Test the Gemini evaluator with sample assignments"""
    ...
    try:
        ...
        sample_files = ["correct_assignment.zip", "incorrect_assignment.zip"]
        
        for file_path in sample_files:
            if os.path.exists(file_path):
                print(f"\n🔍 Evaluating {file_path}...")
                result = evaluator.evaluate_assignment(file_path, "snowflake")
                
                print(f"📊 Overall Score: {result.get('overall_score', 0)}/100")
                print(f"📈 Percentage: {result.get('overall_percentage', 0):.1f}%")
